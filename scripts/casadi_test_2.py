...
class OccupancyGrid2d(object):
    ...
    def SensorCallback(self, pose, map):
        self.pos = map.info.origin.position
        self.ori = map.info.origin.orientation 
        ...
        array = data_reshape[24:27,24:27]
        ...
        for i in range(0,11):  
                ...
                ref_x[i] = (x_min-1)/5+sensor_x
                ref_y[i] = (y_min-1)/5+sensor_y


        self.v, self.w, self.pose = self.controller.compute_control(state_robot, ref_x, ref_y)
        data_reshape[24:27,24:27] = 100
        self.data = data_reshape
        self.Visualize()
    ...
    def Visualize(self):
        ...
        m.info.origin.position = self.pos
        m.info.origin.orientation = self.ori
